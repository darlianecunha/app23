...
POSITIVE_TERMS = [
    r"\bedital(es)?\b", r"\bchamada(s)?\s+p(ú|u)blica(s)?\b",
    r"\bsele(ç|c)[aã]o\s+p(ú|u)blica\b", r"\bretifica(ç|c)[aã]o\s+de\s+edital\b",
    r"\bconvocat(ó|o)ria\b", r"\bportaria\b", r"\bbolsa(s)?\b", r"\bconcurso(s)?\b",
    r"\bfomento\b", r"\bsubmiss(ã|a)o\b", r"\bprorroga(ç|c)[aã]o\b",
    ...
    for nome, url in html_map.items():
        ...
        try:
            ...
            for a in soup.select('a[href]'):
                ...
                itens.append({'fonte':nome,'titulo':texto[:180] or '(sem título)','resumo':'',
                              'link':href,'publicado_em':'','metodo':'HTML'})
        except Exception as ex:
            print(f'[WARN] HTML falhou {nome}: {ex}')
    return itens
def formatar_email(itens):
    if not itens: return f'Nenhum edital/chamada encontrado nos últimos {RECENCIA_DIAS} dias.'
    def key_sort(x): return (x['publicado_em'] or '', x['fonte'], x['titulo'])
    linhas = [f'📢 Editais e Chamadas Públicas (Brasil) – últimos {RECENCIA_DIAS} dias\n']
...
def enviar_email(corpo, assunto=None):
    ...
    if not assunto: assunto = f'📢 Editais (Brasil) – últimos {RECENCIA_DIAS} dias'
...
def main():
    ...
    print(f'🔍 Buscando editais BR (últimos {RECENCIA_DIAS} dias)…')
    itens = coletar_rss(rss_map) + coletar_html(html_map)
    corpo = formatar_email(itens)
    print('\n===== PRÉVIA DO EMAIL =====\n'); print(corpo); print('\n===========================\n')
    try: enviar_email(corpo); print('✅ Email enviado com sucesso!')
    except Exception as e: print('❌ Erro ao enviar e-mail:', e, file=sys.stderr)
